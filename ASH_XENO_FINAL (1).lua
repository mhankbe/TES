...
local function CreateUserProfile(parent)
    ...
    pcall(function()
        if identifyexecutor then execName = identifyexecutor() or "Executor"
        elseif getexecutorname then execName = getexecutorname() or "Executor"
        elseif syn and syn.get_executor_version then execName = "Synapse"
        elseif FLUXUS_EXECUTOR then execName = "Fluxus"
        end
    end)
...
function DoAutoDestroyer(on)
    ...
    _destroyerThread = task.spawn(function()
        ...
        while STATE.autoDestroyer do
            ...
            if not RE.ExtraReward then task.wait(2); do end end
            pcall(function()
...
function DoAutoArise(on)
    ...
    _ariseThread = task.spawn(function()
        ...
        while STATE.autoArise do
            -- [v112-FIX] Guard nil: skip jika remote belum tersedia
            if not RE.ExtraReward then task.wait(2.5); do end end
            pcall(function()
                for _, obj in ipairs(workspace:GetChildren()) do
                    if obj:IsA("Model") or obj:IsA("Part") then
                        local guid = obj:GetAttribute("GUID")
...
function DoMassAttack(on)
 ...
 if MODE.current ~= "idle" and MODE.current ~= "ma" or _raidInterrupt or _siegeInterrupt or (DUNGEON and DUNGEON.interrupt) or (DUNGEON and DUNGEON.inMap) or (ST2 and ST2.running) then WaitRaidDone() end
 if not MA.running then break end
 if _raidInterrupt then _mapIdx = _mapIdx + 1; do end end
...
_animRow.Name = "ZZZ_DisableAnim" -- Mengamankan posisi layout agar tidak berantakan
 ...
 if not guid or #tostring(guid)==0 then do end end
 ...
 local _hook = type(newcclosure) == "function"
 ...
 function StartAutoSellWeapon()
  ...
  -- [XENO FIX] require() tidak didukung di Xeno/Fluxus/Solara
  -- Fallback: gunakan wrapper object yang membaca weapon dari PlayerGui
  local _pm = nil
  pcall(function()
   _pm = require(game:GetService("ReplicatedStorage").Scripts.Client.Manager.PlayerManager)
  end)
  -- Xeno fallback: buat _pm wrapper yang simulate localPlayerData.weapons
  -- dengan scan PlayerGui untuk weaponGuid attribute (sama pola dengan heroGuid)
  if not _pm then
   local _pgWeaponCache = nil
   local _pgWeaponCacheTime = 0
   local function _refreshPGWeapons()
    local now = tick()
    if now - _pgWeaponCacheTime < 0.5 then return _pgWeaponCache end
    local weapons = {}
    pcall(function()
     local function scanObj(obj)
      local wg = obj:GetAttribute("weaponGuid")
          or obj:GetAttribute("WeaponGuid")
          or obj:GetAttribute("guid")
      if type(wg) == "string" and #wg > 20 then
       local entry = {
        guid        = wg,
        itemId      = obj:GetAttribute("itemId") or obj:GetAttribute("ItemId"),
        isLock      = obj:GetAttribute("isLock"),
        isEquip     = obj:GetAttribute("isEquip"),
        isBuffer    = obj:GetAttribute("isBuffer"),
        isFavourite = obj:GetAttribute("isFavourite"),
       }
       weapons[wg] = entry
      end
     end
    ...
    _pgWeaponCache = weapons
    _pgWeaponCacheTime = now
    return weapons
   end
   _pm = {
    localPlayerData = setmetatable({}, {
     __index = function(_, k)
      if k == "weapons" then return _refreshPGWeapons() end
      return nil
     end
    })
   }
  ...
  local function GetSellableWeapons()
   ...
   pcall(function()
    ...
    for guid, data in pairs(weapons) do
     if data.isLock then do end end
     if data.isEquip then do end end
     if data.isBuffer then do end end
     if data.isFavourite then do end end
     ...
     if not _swSelectAll and next(_swSelectedIds) then
      ...
      if not matched then do end end
     end
 ...
 if #guidStr ~= 36 or not guidStr:find("^%x+%-%x+%-%x+%-%x+%-%x+$") then do end end
...
do
 ...
 local r = eRows[nm2]; if not r then do end end
...
ConnectRaidListeners = function()
 ...
 if type(info) ~= "table" then do end end
 local raidId = info.raidId or (type(k)=="number" and k) or tonumber(k)
 local mapId = info.mapId
 if not raidId or not mapId then do end end
 -- Normalize mapId ke lobby range
 if mapId >= 50101 and mapId <= 50119 then mapId = mapId - 100 end
 if mapId < 50001 or mapId > 50019 then do end end
...
-- ============================================================
-- ================================================================
-- ForceRescanRaidEnter — XENO/FLUXUS/SOLARA COMPATIBLE
-- require(RaidsManager) diganti dengan:
--   1. Workspace scan langsung (Maps.Map.RaidEnter.RE1001/RE1002)
--   2. GetRaidTeamInfos invoke (trigger UpdateRaidInfo event dari server)
--   3. Reconciliasi temp-entry RAID_LIVE (sistem existing)
-- Semua logika dan behavior dipertahankan persis seperti versi asli.
-- ================================================================
...
local function ForceRescanRaidEnter()
    ...
    pcall(function()
        ...
        -- Sama dengan data yang disediakan RaidsManager, tanpa require.
        ...
        if reF then
            for _, slotFolder in ipairs(reF:GetChildren()) do
                ...
                if not SPAWN_RANK[slotName] then do end end

                for _, child in ipairs(slotFolder:GetChildren()) do
                    local mapNum = _parseRaidEnterName(child.Name)
                    if not mapNum or mapNum < 1 or mapNum > 19 then do end end
                    ...
                    if not raidId then
                        pcall(function()
                            local rv = child:FindFirstChild("RaidId") or child:FindFirstChild("raidId")
                    ...
                    local endTime = child:GetAttribute("endTime")
                        ...
                        or child:GetAttribute("end_time")
                    ...
                    if type(raidId) == "number" and raidId > 0 then
                        -- Normalize mapId jika perlu
                        if mapId >= 50101 and mapId <= 50119 then mapId = mapId - 100 end
                        if mapId < 50001 or mapId > 50019 then do end end

                        -- Upgrade grade via RAID_CONFIG_GRADE (akurat 100%)
                        ...
                        if not RAID_LIVE[raidId] then
                            ...
                            -- Auto-update grade/rank jika ada perubahan
                        ...
                        -- raidId belum ada di portal → buat temp entry (sama seperti _onRaidChildAdded)
                        local tempKey = -(mapId)
                        local hasReal = false
                        for _, ent in pairs(RAID_LIVE) do
        ...
        -- Server merespons GetRaidTeamInfos dengan mengirim UpdateRaidInfo,
        -- yang sudah ditangkap oleh ConnectRaidListeners().
        -- Ini menggantikan "brain hijack" RaidsManager tanpa require.
        task.spawn(function()
        ...
        -- ── Rebuild list dan notif jika ada perubahan ─────────────────
        if newFound then
            RebuildRaidList()
            if _raidWakeup then pcall(function() _raidWakeup:Fire() end) end
...
-- Radar global berjalan otomatis setiap 1.5 detik (sama seperti versi asli)
task.spawn(function()
...
StartSiegeLoop = function()
    ...
    SIEGE.thread = task.spawn(function()
        while SIEGE.running do
            ...
            local targetMap = nil
            for _, mn in ipairs(SIEGE_MAP_NUMS) do
                if SIEGE.excludeMaps and SIEGE.excludeMaps[mn] then do end end
 ...
 local function GlobalResetConfig()
  ...
  pcall(function()
   if _siegeItemRefs then
    for _, mn in ipairs(SIEGE_MAP_NUMS) do
     local r = _siegeItemRefs[mn]; if not r then do end end
...
do
 ...
 if not ok then task.wait(0.5); do end end
...
do
    local function SetupUniversalSpy()
        ...
        if type(getrawmetatable) == "function" then
            local mt = getrawmetatable(game)
            local _old = mt and mt.__namecall
            
            if mt and _old then
                if type(setreadonly) == "function" then setreadonly(mt, false) end
                local _wrapFn = function(self, ...)
                    local _m = type(getnamecallmethod) == "function" and getnamecallmethod() or ""
                ...
                mt.__namecall = type(newcclosure) == "function" and newcclosure(_wrapFn) or _wrapFn
                if type(setreadonly) == "function" then setreadonly(mt, true) end
            end
        end
    end
