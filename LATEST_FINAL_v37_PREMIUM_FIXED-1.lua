...
do
    ...
    function StopWRX100()
        _WR_RPT.x100 = false
        if _wx100ToggleEl then _wx100ToggleEl:Set(false) end
        if _WR_RPT.x100Thread then
            pcall(function() task.cancel(_WR_RPT.x100Thread) end)
            _WR_RPT.x100Thread = nil
        end
...
do
    ...
    _PGR_RPT = {
        ...
        statusEls     = {nil, nil, nil},  -- Paragraph: GUID status per mesin
        rollEls       = {nil, nil, nil},  -- Paragraph: status roll live + attempt
        lastEls       = {nil, nil, nil},  -- Paragraph: hasil roll terakhir
    ...
    for msi = 1, 3 do
        ...
        -- GUID status (nama mesin sebagai Title)
        _PGR_RPT.statusEls[msi_l] = pgrSection:Paragraph({
            Title = PG_MACHINE_NAMES[msi_l],
            Desc  = "[..] REROLL 1x dulu di Mesin",
        })

        -- Status roll live (attempt / target preview)
        ...
        -- Target dropdown (multi-select grade khusus mesin ini)
        local ddEl, _ = pgrSection:Dropdown({
            ...
            Value    = {},
            Callback = function(selected)
                -- rebuild targets dari selected names (sama pola dengan Weapon)
                local tbl = {}
                for _, g in ipairs(PG_GRADES_PER_MACHINE[msi_l]) do
                    for _, selName in ipairs(selected) do
        ...
        -- Capture refresh fn untuk Config restore (future use)
        ...
        -- Toggle Fastroll normal (1.lua baris 8118-8135)
        local toggleEl = pgrSection:Toggle({
            Title    = "Fastroll " .. PG_MACHINE_NAMES[msi_l],
            Desc     = "ON = START REROLL",
            Value    = false,
            Callback = function(on)
                DoAutoRollPetGear(msi_l, on)
            end,
        })
        _PGR_RPT.toggleEls[msi_l] = toggleEl

        -- Toggle 100x Reroll (1.lua baris 8137-8206)
        local x100El = pgrSection:Toggle({
            ...
            Callback = function(on)
                ...
                if on then
                    -- Stop Fastroll biasa dulu kalau aktif (1.lua baris 8186-8193:
                    -- tidak boleh 2 loop jalan bersamaan di slot yang sama)
                    if _PGR_RPT.running[msi_l] then
                        _PGR_RPT.running[msi_l] = false
                        if _PGR_RPT.toggleEls[msi_l] then _PGR_RPT.toggleEls[msi_l]:Set(false) end
    ...
    -- Expose ke global Config (future use, pola sama dgn _setWeaponRollToggle)
    ...
    -- ── AUTO ROLL LOGIC NORMAL - PET GEAR (dari 1.lua baris 19858-20018) ─────
    local LOOPS_PG = {}
    ...
    local function StartPetGearSlot(si)
        ...
        if not (_PGR_RPT.guids[si] and _PGR_RPT.guids[si] ~= "") then
            ...
            -- Tunggu GUID ter-capture lalu auto-start (1.lua baris 19888-19897)
            task.spawn(function()
        ...
        LOOPS_PG[si] = task.spawn(function()
            while _PGR_RPT.running[si] do
                repeat
                    ...
                    -- Parse gradeId rekursif (dari 1.lua baris 19938-19963)
                    local gotId = nil
                    if type(res) == "table" then
                    ...
                    -- Hanya stop kalau target dipilih DAN hasil cocok (1.lua baris 19966)
                    local hit = gotId and hasTarget and targets[gotId] == true
    ...
    DoAutoRollPetGear = function(si, on)
        ...
        _PGR_RPT.running[si] = on
        if not on then
            -- [v103] Reset GUID saat toggle OFF - wajib Reroll 1x lagi (1.lua baris 19995-20001)
            _PGR_RPT.guids[si] = ""
        ...
        if not hasTarget then
            _PGR_RPT.SetRoll(si, "[!] SELECT TARGET PLEASE!")
            -- Tetap jalankan slot, dia akan loop tunggu target (1.lua baris 20007-20016)
        end
        StartPetGearSlot(si)
    end

    -- ── AUTO ROLL LOGIC 100x - PET GEAR (dari 1.lua baris 3340-3501) ─────────
    local function SetOff100(si)
        _PGR_RPT.x100[si] = false
        if _PGR_RPT.x100ToggleEls[si] then _PGR_RPT.x100ToggleEls[si]:Set(false) end
    ...
    function StartPG100Loop(si)
        ...
        _PGR_RPT.x100Thread[si] = task.spawn(function()
            ...
            -- Validasi: harus ada GUID (1.lua baris 3379-3393)
            if not (_PGR_RPT.guids[si] and _PGR_RPT.guids[si] ~= "") then
                _PGR_RPT.SetRoll(si, "[..] Click 1x on Reroll Machine")
            ...
            -- Validasi: harus ada target (1.lua baris 3395-3399)
            ...
            while _PGR_RPT.x100[si] do
                repeat
                    -- [FIX] Cek GUID tiap iterasi (1.lua baris 3404-3408)
                    if not (_PGR_RPT.guids[si] and _PGR_RPT.guids[si] ~= "") then
                        _PGR_RPT.SetRoll(si, "[..] Click 1x on Reroll Machine")
                        task.wait(1); break
                    end

                    -- [FIX] Rebuild stopIds tiap iterasi (1.lua baris 3410-3418)
                    ...
                    -- Parse gradeId rekursif deep scan (1.lua baris 3448-3476)
                    local gotId = nil
                    if type(res) == "table" then
                        gotId = res.gradeId or res.grade or res.id or res.resultId
                        if type(gotId) ~= "number" and type(res.data) == "table" then
                        ...
                        if type(gotId) ~= "number" then
                            local function FindGradeId100(t, depth)
                                ...
                                for k, v in pairs(t) do
                                    if type(v) == "number" and v > 0 then
                                        if _PGR_RPT.targets[si][v] then return v end
    ...
    -- ── Heartbeat poller Pet Gear Fastroll (3 mesin independen) ──────────────
    RunService.Heartbeat:Connect(function()
        for si = 1, 3 do
