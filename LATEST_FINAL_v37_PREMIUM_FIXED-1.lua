...
local function _WH_resolveGrade(ent)
...
_WH.SendSiege = function(url)
 ...
 if SIEGE and SIEGE.live then
  for _, mn in pairs(SIEGE.live) do
   table.insert(lines, "- Map "..mn.." - "..(SIEGE_NAMES[mn] or "Map "..mn))
   found = true
  end
 end
...
-- [v262 FIX] JANGAN set inMap=true dulu sebelum raidMapId di-assign
 ...
 local connS, connF
 -- [BUG FIX 1&2] _raidDone hanya diset SETELAH attack loop selesai.
 -- ChallengeRaidsSuccess tidak boleh interrupt attack di tengah jalan.
 -- _raidServerDone = flag bahwa server sudah bilang sukses (tapi attack tetap jalan sampai boss mati)
...
StartSiegeLoop = function()
    ...
    SIEGE.thread = task.spawn(function()
        while SIEGE.running do
            ...
            -- [FIX SIEGE PANEL] Restore panel Siege/CityRaid yang sudah terlanjur di-hide
            -- oleh _hideUIOn sebelum SIEGE.inMap = true di-set.
            -- Tanpa ini: panel Count/Timer/Reward tidak tampil -> reward tidak trigger.
            pcall(function()
                local _SIEGE_KW = {
                    "cityraid","city_raid","garrisoncityraid","garrisonboss",
                    "siege","cityraidpanel","cityraidenterpanel",
                ...
                for _, gui in ipairs(PG:GetChildren()) do
                    pcall(function()
                        ...
                        local n = gui.Name:lower()
                        local isSiege = false
                        for _, kw in ipairs(_SIEGE_KW) do
                            if n:find(kw, 1, true) then isSiege = true; break end
                        end
                        if isSiege then
                            if gui:IsA("ScreenGui") or gui:IsA("GuiBase2d") then
                                gui.Enabled = true
...
local function DungeonTpIn()
 -- [FIXED] Match with SimpleSpy capture - complete remote sequence
...
StartDungeonLoop = function()
 ...
 -- [FIXED] Tunggu konfirmasi masuk max 15 detik - rely only on MapId check
 local _entered = false
 local _entWait = 0
 ...
 -- [REMOVED] Enemy fallback check - causes script to attack enemies OUTSIDE dungeon
 -- Old buggy code: if #GetSiegeEnemies() > 0 then _entered = true; break end
 end
    ...
    local _, _setAnnivRunToggle = ToggleRow(inner, "Run", "Jalankan loop Anniversary", 3, function(on)
        ...
        if on then
            ...
            ANNIV.thread = task.spawn(function()
                ...
                -- ── Helper: Ambil semua Trainee Angel hidup di Workspace.Enemys ──
                local function GetTraineeAngels()
                    local list = {}
                    local ef = workspace:FindFirstChild("Enemys")
                    if not ef then return list end
                    for _, e in ipairs(ef:GetChildren()) do
                        if not e:IsA("Model") then continue end
                        if e.Name ~= "Trainee Angel" then continue end
                        local g   = e:GetAttribute("EnemyGuid") or e:GetAttribute("BossGuid") or e:GetAttribute("Guid") or e:GetAttribute("GUID")
                        local h   = e:FindFirstChild("HumanoidRootPart")
                        local hum = e:FindFirstChildOfClass("Humanoid")
                ...
                -- ── Helper: Teleport karakter ke RaidsEnemys.4035 ──
                local function TpToAnnivEnemy()
                    ...
                    if not mapsFolder then return false end
                    local mapAnniv = mapsFolder:FindFirstChild("MapAnniversary")
                ...
                -- ════════════════════════════════════════════════════════
                -- WHILE LOOP SEJATI — berjalan terus selama ANNIV.running
                -- ════════════════════════════════════════════════════════
                while ANNIV.running do
                    ...
                    AnnivStatus("[1/6] Checking reward count...", Color3.fromRGB(240,165,0))
                    ...
                    AnnivStatus("[2/6] Creating raid team...", Color3.fromRGB(240,165,0))
                    ...
                    AnnivStatus("[3/6] Re-checking reward count...", Color3.fromRGB(240,165,0))
                    pcall(function()
                        Remotes.GetActivityRaidRewardCount:InvokeServer(RAID_ID)
                    end)
                    task.wait(0.5)
                    ...
                    -- Step 4 : UseItem (Anniversary Map Entries+1 ticket)
                    AnnivStatus("[4/6] Using entry ticket (itemId "..ITEM_ID..")...", Color3.fromRGB(240,165,0))
                    ...
                    AnnivStatus("[5/6] Re-checking after ticket use...", Color3.fromRGB(240,165,0))
                    pcall(function()
                        Remotes.GetActivityRaidRewardCount:InvokeServer(RAID_ID)
                    end)
                    task.wait(0.5)
                    ...
                    -- Step 7 : StartLocalPlayerTeleport → masuk map anniversary
                    ...
                    AnnivStatus("[OK] Masuk Anniversary Gate! Jeda 2s...", Color3.fromRGB(46,204,64))
                    task.wait(2)
                    if not ANNIV.running then break end

                    -- ── PHASE 2: TELEPORT KE MUSUH (pertama kali) ───────────
                    AnnivStatus("[TP] Teleport ke RaidsEnemys.4035...", Color3.fromRGB(240,165,0))
                    local tpOk = false
                    for i = 1, 3 do
                        if not ANNIV.running then break end
                        if TpToAnnivEnemy() then tpOk = true; break end
                        AnnivStatus("[TP] Tunggu RaidsEnemys.4035... ("..i.."/3)", Color3.fromRGB(240,165,0))
                        task.wait(1)
                    end
                    if not ANNIV.running then break end
                    if not tpOk then
                        AnnivStatus("[X] RaidsEnemys.4035 tidak ditemukan, skip run...", Color3.fromRGB(200,50,50))
                        -- Lanjut cooldown lalu loop ulang (tidak stop total)
                        AnnivStatus("[..] Cooldown 5s lalu ulang...", Color3.fromRGB(100,180,255))
                        task.wait(5)
                        continue
                    end

                    -- ── PHASE 3: JEDA 1s lalu UnEquipAll + BestEquip ────────
                    task.wait(1)
                    if not ANNIV.running then break end
                    ...
                    -- ── PHASE 4: JEDA 1s lalu ATTACK LOOP ───────────────────
                    -- Sprint + Teleport ke musuh diulang setiap 2 detik
                    -- Selesai jika jumlah "Trainee Angel" hidup berkurang dari 9
                    task.wait(1)
                    if not ANNIV.running then break end

                    AnnivStatus("[ATK] Menyerang Trainee Angel...", Color3.fromRGB(240,165,0))

                    -- Tunggu musuh muncul (maks 8 detik)
                    local waitT = 0
                    while waitT < 8 and ANNIV.running do
                        if #GetTraineeAngels() > 0 then break end
                        AnnivStatus("[ATK] Nunggu Trainee Angel spawn... ("..math.floor(8-waitT).."s)", Color3.fromRGB(240,165,0))
                        task.wait(0.5); waitT = waitT + 0.5
                    end
                    if not ANNIV.running then break end

                    -- Catat jumlah awal Trainee Angel
                    local initialCount = #GetTraineeAngels()
                    local stuckTimer   = 0
                    local STUCK_LIMIT  = 30.0  -- 30 detik stuck limit

                    while ANNIV.running do
                        local alive      = GetTraineeAngels()
                        local aliveCount = #alive

                        -- Kondisi selesai: ada Trainee Angel yang mati
                        if aliveCount < initialCount then
                            AnnivStatus("[OK] Trainee Angel mati! ("..aliveCount.."/"..initialCount..") Ambil reward...", Color3.fromRGB(46,204,64))
                            break
                        end

                        -- Teleport karakter ke musuh
                        TpToAnnivEnemy()

                        -- Sprint remotes
                        pcall(function() Remotes.PlaySprintSkill:InvokeServer() end)
                        pcall(function() Remotes.PlaySprintSkillEffect:FireServer() end)
                        pcall(function() Remotes.StopSprintSkillEffect:FireServer() end)

                        -- Anti-stuck check
                        stuckTimer = stuckTimer + 2
                        if stuckTimer >= STUCK_LIMIT then
                            AnnivStatus("[!] Stuck "..STUCK_LIMIT.."s, lanjut ke reward...", Color3.fromRGB(200,100,50))
                            break
                        end

                        AnnivStatus("[ATK] Sprint... ("..aliveCount.." Trainee Angel sisa)", Color3.fromRGB(240,165,0))
                        task.wait(2)
                    end

                    if not ANNIV.running then break end

                    -- ── PHASE 5: JEDA 1s — instant collector berjalan background ─
                    AnnivStatus("[REWARD] Jeda 1s ambil reward...", Color3.fromRGB(46,204,64))
                    StartInstantGoldCollector(true)
                    task.wait(1)
                    if not ANNIV.running then break end

                    -- ── PHASE 6: COOLDOWN 5s lalu loop ulang dari Step 1 ────────
                    AnnivStatus("[CD] Cooldown 5s lalu ulang...", Color3.fromRGB(100,180,255))
                    for i = 5, 1, -1 do
                        if not ANNIV.running then break end
                        AnnivStatus("[CD] Loop ulang dalam "..i.."s...", Color3.fromRGB(100,180,255))
                        task.wait(1)
                    end
                    if not ANNIV.running then break end

                    AnnivStatus("[LOOP] Mulai ulang Anniversary...", Color3.fromRGB(240,165,0))
                    -- while loop kembali ke atas (Step 1) secara otomatis

                end -- end while ANNIV.running

                -- Di sini hanya tercapai jika toggle OFF
                AnnivStatus("[STOP] Anniversary STOP TOTAL.", Color3.fromRGB(100,100,100))
                ANNIV.thread = nil
            end)
        else
            -- Toggle OFF — STOP TOTAL
            ANNIV.running = false
...
do
 ...
 -- CONFIG FILE PATH
 -- ============================================================
 local CONFIG_PATH = "FLaProjectConfig.json"
 ...
 local function LoadConfig()
  local ok, result = pcall(function()
   if not isfile(CONFIG_PATH) then return nil, "File tidak ditemukan" end
   local raw = readfile(CONFIG_PATH)
   if not raw or raw == "" then return nil, "File kosong" end
   local cfg = jsonDecode(raw)
   if type(cfg) ~= "table" then return nil, "Format config tidak valid" end
   return cfg
  end)
 ...
 local function HasConfig()
  local ok, res = pcall(function() return isfile(CONFIG_PATH) end)
 ...
 local hdrSub = Label(hdrCard, "Save dan load semua state fitur yang aktif ke file.", 10, C.DIM, Enum.Font.Gotham, Enum.TextXAlignment.Left)
 ...
 local statusLbl = Label(statusCard, "Belum ada config tersimpan.", 11, C.TXT2, Enum.Font.Gotham, Enum.TextXAlignment.Left)
 ...
 -- Cek awal apakah config sudah ada
 if HasConfig() then
  SetStatus("Config ditemukan. Klik Load Config untuk menerapkan.", C.ACC2)
 end

 -- ── 3 tombol utama ───────────────────────────────────────
 local btnRow = Frame(p, C.BLACK, UDim2.new(1,0,0,48))
 btnRow.BackgroundTransparency = 1; btnRow.LayoutOrder = 2
 local brl = New("UIListLayout",{Parent=btnRow, FillDirection=Enum.FillDirection.Horizontal,
 ...
 local lblSave = Label(btnSave,"SAVE CONFIG",11,Color3.fromRGB(255,255,255),Enum.Font.GothamBold,Enum.TextXAlignment.Center)
 ...
 local lblLoad = Label(btnLoad,"LOAD CONFIG",11,C.ACC2,Enum.Font.GothamBold,Enum.TextXAlignment.Center)
 ...
 -- ── Preview card ─────────────────────────────────────────
 local previewCard = Frame(p, C.SURFACE, UDim2.new(1,0,0,0))
 previewCard.LayoutOrder = 3; Corner(previewCard,10); Stroke(previewCard,C.BORD,1.5,0.7)
 previewCard.AutomaticSize = Enum.AutomaticSize.Y
 Padding(previewCard, 10, 10, 12, 12)
 local previewLayout = New("UIListLayout",{Parent=previewCard, SortOrder=Enum.SortOrder.LayoutOrder, Padding=UDim.new(0,4)})

 local previewTitle = Label(previewCard,"PREVIEW ISI CONFIG",10,C.TXT3,Enum.Font.GothamBold,Enum.TextXAlignment.Left)
 previewTitle.Size = UDim2.new(1,0,0,14); previewTitle.LayoutOrder = 0

 local previewLines = {}
 local function MakePreviewLine(order)
  local lbl = Label(previewCard,"",10,C.TXT2,Enum.Font.Gotham,Enum.TextXAlignment.Left)
  lbl.Size = UDim2.new(1,0,0,13); lbl.LayoutOrder = order
  lbl.TextTruncate = Enum.TextTruncate.AtEnd
  return lbl
 end
 ...
 local function RefreshPreview()
  ...
  if not HasConfig() then
   local l = MakePreviewLine(1)
   l.Text = "  Belum ada config tersimpan."
   l.TextColor3 = C.TXT3
  ...
  if type(cfg) ~= "table" then
   local l = MakePreviewLine(1)
   l.Text = "  Gagal membaca config."
   l.TextColor3 = Color3.fromRGB(220,100,100)
   table.insert(previewLines, l)
   return
  end

  local items = {
   {"Main - Auto Sell Hero Equip",   cfg.sellHeroOn},
   {"Main - Auto Collect Gold/Item", cfg.autoCollectOn},
   {"Main - Auto Sell Weapon",       cfg.sellWeaponOn},
   {"Main - Auto Decomp Gem",        cfg.decompGemOn},
   {"Hide - Hide Reroll Chat",       cfg.hideRerollChat},
   {"Hide - Hide All UI",            cfg.hideAllUI},
   {"Hide - Hide All Anim",          cfg.hideAllAnim},
   {"Hide - Auto Hide Reward",       cfg.hideReward},
   {"Farm - Random Attack",          cfg.randomAttackOn},
   {"Attack - Mass Attack",          cfg.massAttackOn},
   {"Attack - Skill Z",              cfg.skillZ},
   {"Attack - Skill X",              cfg.skillX},
   {"Attack - Skill C",              cfg.skillC},
   {"Attack - Skill V",              cfg.skillV},
   {"Attack - Skill F",              cfg.skillF},
   {"Player - No Clip",              cfg.noClipOn},
   {"Player - Anti AFK",             cfg.antiAfkOn},
   {"Player - Walk Speed",           cfg.walkSpeed and tostring(cfg.walkSpeed) or "16"},
   {"Automation - Auto Raid",        cfg.raidOn},
   {"Automation - Raid Pick Mode",   cfg.raidPMIdx and ({[1]="Default",[2]="By Rank",[3]="By Map",[4]="Hard",[5]="Easy",[6]="Manual"})[cfg.raidPMIdx] or "Default"},
   {"Automation - Raid Updown",      cfg.raidUpdownEnabled},
   {"Automation - Raid Updown Dir",  cfg.raidUpdownDir or "up"},
   {"Automation - Raid Updown Grade",cfg.raidUpdownTargetGrade or "none"},
   {"Automation - Raid Rune Map",    cfg.raidRuneMapTarget and tostring(cfg.raidRuneMapTarget) or "0"},
   {"Automation - Raid List Mode",   cfg.raidListEnabled},
   {"Automation - Auto Asc",         cfg.ascOn},
   {"Automation - Auto Siege",       cfg.siegeOn},
   {"Automation - Auto Dungeon",     cfg.dungeonOn},
   {"Automation - Auto ST2",         cfg.st2On},
   {"Reroll - Hero Fastroll",        cfg.heroRollOn},
   {"Reroll - Hero x100",            cfg.heroX100On},
   {"Reroll - Weapon Fastroll",      cfg.weaponRollOn},
   {"Reroll - Weapon x100",          cfg.weaponX100On},
   {"Reroll - Merge Potion",         cfg.mergeOn},
   {"Reroll - Use Potion",           cfg.useOn},
   {"Main - Sell Weapon (All?)",     cfg.swSelectAll},
   {"Main - Decomp Gem Min",         cfg.gemMinLevel and tostring(cfg.gemMinLevel) or "1"},
  ...
  if cfg.pgrOn then
   for i=1,3 do
    table.insert(items, {"Reroll - PetGear Slot "..i, cfg.pgrOn[i]})
   end
  end
  -- Halo
  if cfg.haloOn then
   for i=1,3 do
    table.insert(items, {"Reroll - Halo "..i, cfg.haloOn[i]})
   end
  end
  -- Ornament
  if cfg.ornOn then
   for i=1,#cfg.ornOn do
    table.insert(items, {"Reroll - Ornament "..i, cfg.ornOn[i]})
   end
  end

  for i,item in ipairs(items) do
   local l = MakePreviewLine(i)
   local label = item[1]
   local val   = item[2]
   local valStr
   ...
   l.Text = "  "..label..":  "..valStr
   l.TextColor3 = (val == true) and C.ACC2 or C.TXT3
   table.insert(previewLines, l)
  end
 end
 ...
 btnSave.MouseButton1Click:Connect(function()
  ...
  _saving = true
  lblSave.Text = "SAVING..."
  SetStatus("Menyimpan config...", Color3.fromRGB(255,200,60))
  task.delay(0.1, function()
   local ok, err = SaveConfig()
   _saving = false
   if ok then
    lblSave.Text = "SAVE CONFIG"
    SetStatus("Config tersimpan. ("..os.date("%H:%M:%S")..")", C.ACC2)
    RefreshPreview()
   else
    lblSave.Text = "SAVE CONFIG"
    SetStatus("[!] Gagal menyimpan: "..(tostring(err):sub(1,40)), Color3.fromRGB(220,100,100))
   end
  end)
 end)
 ...
 btnLoad.MouseButton1Click:Connect(function()
  ...
  SetStatus("Memuat config...", Color3.fromRGB(255,200,60))
  task.delay(0.1, function()
   local cfg, err = LoadConfig()
   if type(cfg) ~= "table" then
    _loading = false
    lblLoad.Text = "LOAD CONFIG"
    SetStatus("[!] "..(err or "File tidak ditemukan atau rusak."), Color3.fromRGB(220,100,100))
    return
   end
   local ok2 = ApplyConfig(cfg)
   _loading = false
   lblLoad.Text = "LOAD CONFIG"
   if ok2 then
    SetStatus("Config berhasil diterapkan. ("..os.date("%H:%M:%S")..")", C.ACC2)
   else
    SetStatus("[!] Partial load — beberapa setter belum siap.", Color3.fromRGB(255,180,60))
   end
  end)
 end)

 local _confirmDel = false
 local _confirmTimer = nil
 btnDel.MouseButton1Click:Connect(function()
  if not _confirmDel then
   _confirmDel = true
   lblDel.Text = "YAKIN?"
   lblDel.TextColor3 = Color3.fromRGB(255,80,80)
   SetStatus("Klik DELETE sekali lagi untuk konfirmasi hapus config.", Color3.fromRGB(255,140,60))
   if _confirmTimer then task.cancel(_confirmTimer) end
   _confirmTimer = task.delay(4, function()
    _confirmDel = false
    lblDel.Text = "DELETE"
    lblDel.TextColor3 = Color3.fromRGB(220,100,100)
    SetStatus(HasConfig() and "Config ditemukan. Klik Load Config untuk menerapkan." or "Belum ada config tersimpan.", HasConfig() and C.ACC2 or C.TXT2)
   end)
  else
   _confirmDel = false
   if _confirmTimer then pcall(function() task.cancel(_confirmTimer) end) end
   lblDel.Text = "DELETE"
   lblDel.TextColor3 = Color3.fromRGB(220,100,100)
   local ok = DeleteConfig()
   if ok then
    SetStatus("Config dihapus dari workspace.", Color3.fromRGB(220,100,100))
    RefreshPreview()
   else
    SetStatus("[!] Gagal menghapus config.", Color3.fromRGB(220,100,100))
   end
  end
 end)
