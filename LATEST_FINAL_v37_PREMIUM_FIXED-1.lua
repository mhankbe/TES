...
do
  ...
  local tChar = task.spawn(function()
   while RA.running do
    ...
    if RA.cur or (TA.running and TA.cur) then
     task.wait(0.5)  -- 10x/detik
    else
     task.wait(0.5)  -- [FIX] percepat: tidak lagi 0.2 agar cepat scan musuh baru
    end
   end
  end)
  RA.threads = {tChar}
  ...
  local tChar = task.spawn(function()
   ...
   while TA.running do
    ...
    if not IsDeadF(tgt) and tgt.model.Parent then
     ...
     task.wait(0.5)  -- [FIX] percepat polling mati
    end
   end
  end)
  TA.threads = {tChar}
  ...
  local tChar = task.spawn(function()
   ...
   while TA.running do
    ...
    if #pool == 0 then
     ...
     while TA.running do
      task.wait(0.5)  -- [FIX] lebih cepat scan respawn
    ...
    if not tgt or IsDeadF(tgt) then
     ...
     rrIdx = rrIdx + 1
     task.wait(0.5)  -- [FIX] percepat skip mati
    else
     TA.cur = tgt
     _curDied = false
 ...
 refBtn.MouseButton1Click:Connect(function()
  refBtn.Text = "Loading..."
  task.spawn(function() RefreshEnemies(); task.wait(0.5); refBtn.Text = "Refresh" end)
 end)
end
...
-- [v262 FIX] JANGAN set inMap=true dulu sebelum raidMapId di-assign
 ...
 local MAX_DIST_BOSS = 2000 -- [FIX ZOMBIE] Diperkecil dari 6000; boss zombie dari map lain umumnya di luar radius ini
